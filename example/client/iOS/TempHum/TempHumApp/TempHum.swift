...
class TempHum : ObservableObject{
    var addTimer: Timer!
    @Published var data = (temp:"", hum:"", press:"")
            ...
            if let error = error {
                ...
                if let data = data {
                    ...
                    DispatchQueue.main.async {
                        self.data.temp = "温度:\(words[0].trimmingCharacters(in: .whitespacesAndNewlines))"
                        self.data.press = "気圧:\(words[1].trimmingCharacters(in: .whitespacesAndNewlines))"
                        self.data.hum = "湿度:\(words[2].trimmingCharacters(in: .whitespacesAndNewlines))"
                    }
                    semaphore.signal()
                }
                else {
